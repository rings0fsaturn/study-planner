...
- `AGENTS.md`, `AGENTS.md`, `.cursor/rules/`, `.github/copilot-instructions.md` — explicit project conventions
...
>
> **Before reviewing the code, inspect the project to determine the standards bar.** Read whichever of these exist: `AGENTS.md`, `AGENTS.md`, dependency manifests (`package.json`, `pom.xml`, `build.gradle`, `go.mod`, `Cargo.toml`, `requirements.txt`, `pyproject.toml`, etc.), lint/format configs, CI config, and a sampling of existing code in the same area. The standards you apply must match this project, not generic best-practice advice from your training data.
>
...
- `standards-violation` — contradicts a rule from `AGENTS.md` / lint config / project convention
