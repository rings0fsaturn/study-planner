#!/usr/bin/env python3
"""
regen-toc.py — rebuild .claude/memory/toc.md from index.jsonl.

Picks flat mode below FLAT_THRESHOLD rows, directory-index mode above.
Also writes per-directory ToCs to .claude/memory/toc/<dirname>.md when
in directory-index mode.

Appends the Flows section from .claude/memory/flows/*.md frontmatter.
...
def write_flat_toc(rows: list[dict], flows: list[dict], stale_pct: float):
    ...
    parts.append("")
    if not rows:
        parts.append("(empty — index grows as Claude reads files)")
        parts.append("")
    else:
...
def write_directory_index_toc(rows: list[dict], flows: list[dict], stale_pct: float):
    ...
    parts.append(
        ...
        f"For details on a dir, read `.claude/memory/toc/<dir>.md`._"
    )
    parts.append("")
