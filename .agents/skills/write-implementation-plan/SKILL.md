...
description: >-
  Consolidate a planning discussion into a durable, agent-handoff-ready implementation plan at `plans/<YYYY-MM-DD>-<slug>.md`. Capture the agreed decisions, rejected options, vertical-slice phases, exact file paths, code references, tests, and verification commands needed for a fresh agent to execute the work phase by phase. Use whenever the user says "write an implementation plan", "convert this to a plan", "spec this out for an agent to build", "write up what we decided", "create an execution plan", "turn this into a plan an agent can execute", "draft an impl plan", "write the plan", or a planning conversation is being formalized for handoff. Pairs with grill-me upstream: grill -> agree -> write-implementation-plan.
---
...
- Implicit assumptions Codex made that the user never explicitly confirmed
...
If the user says "just write it" — fine. Each unresolved item becomes an entry in the Open Questions section of the plan, AND any decision Codex made implicitly gets `🤔 Assumed (unconfirmed)` status in the Decisions log. Don't pretend they were resolved.
...
- `🤔 Assumed (unconfirmed)` — Codex proposed it, user didn't object but never explicitly confirmed. Surface these in the pre-write sanity sweep so user has a chance to upgrade or push back. **Always log assumed decisions — don't omit them.** The implementing agent needs to see them so they don't accidentally treat them as firm.
