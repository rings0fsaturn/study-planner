...
Where `<model name>` is the specific AI model used (e.g., `Codex Sonnet 4.5`, `GPT-4`, `Codex Haiku 4.5`).
...
Co-authored-by: GitHub Copilot using Codex Sonnet 4.5"
```
...
```powershell
git commit -m "feat(scope): summary`n`nBody text`n`nCo-authored-by: GitHub Copilot using Codex Sonnet 4.5"
```
