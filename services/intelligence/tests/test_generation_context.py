...
def test_build_context_returns_retrieved_chunks(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    chunks = build_context(
        "m1", ("core", "recall"), "Book title", "https://supabase.example", "svc-key", rest
...
def test_build_context_steer_falls_back_to_title(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    build_context("m1", (), "Lone title", "https://supabase.example", "svc-key", rest)  # type: ignore[arg-type]
    assert embedded == [["Lone title"]]
...
def test_build_context_embedder_unavailable_is_retryable(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(IngestionError) as excinfo:
        build_context("m1", ("core",), "Title", "https://supabase.example", "svc-key", rest)  # type: ignore[arg-type]
