...
| 4. Column + guard + script | ✅ code / 🟡 push pending | Migration `017_material_embedding_provider.sql` (column + CHECK + comment) committed; **push deferred**: this host has no supabase CLI (rule 36's `~/.local/bin/supabase` is absent) and no DB password / access token, so the live push must run on the user's normal channel (`supabase db push` or dashboard SQL editor). `reembed_materials.py` written: nulls chunk vectors, resets skipped, tags provider, creates attempt+1 job, enqueues embed; refuses non-ready materials and no-ops on same provider. |
...
| 7 | reembed script | ✅ written; live run pending migration push |
...
- Migration push could not run on this host (supabase CLI and database
  credentials absent). Documented as a pending operator step rather than
  guessing a connection path.
