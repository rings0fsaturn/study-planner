...
| [APP][RESEARCH] | scoped-question-generation | GitHub **#62** filed + claimed 2026-09-11; **P1 done** (steer is no longer the material title, `META_BLOCKLIST` in both prompts, empty steer refused); **P2 done** (page provenance + migration 029: live 754/754 chunks paged 1..572, page-bounded RPC verified inclusive); **P3 done** (outline at ingestion via `app/ingestion/outline.py` + migration 030: live re-ingest stores 16 chapters, `page_offset` -33, `page_count` 572, equal to the printed contents page); branch `phase2/issue-62-scoped-question-generation` off `237faf8`; next P4 (scope contract + picker + scoped retrieval). | active/scoped-question-generation/state.md |
