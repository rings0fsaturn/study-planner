...
| [APP][RESEARCH] | scoped-question-generation | GitHub **#62** filed + claimed 2026-09-11; **P1–P5 done** (steer fix + `META_BLOCKLIST`; page provenance + migration 029; outline at ingestion + migration 030; scope contract + chapter picker + scoped/spread retrieval, no new migration; pdf.js viewer at `/materials/:id/view` with a signed URL and a range handoff to the config); P5 verified live — viewer 2/2 at 1280/375 with real canvas ink, chapter jump to pdf 156, `Assess pages 156–213` → config → grounded question, plus the authenticated API gate with its citation inside the range; **AC1–AC6 all ticked**. One measured finding recorded, not fixed: every viewer open downloads the whole 22.9 MB because Supabase Storage does not expose `Accept-Ranges` over CORS (the explicit range transport measured worse and was reverted). Wayfinder exit (push, comment, close, map line, archive) still open. Branch `phase2/issue-
