...
- Not run: the authenticated `POST /v1/assessments/generate` click-through — `.work/specs/test-login-cred.txt` no longer satisfies Supabase's password grant (`invalid_credentials`), so that route is covered by the 2 new API tests and the generation proof drives the same worker objects directly.
