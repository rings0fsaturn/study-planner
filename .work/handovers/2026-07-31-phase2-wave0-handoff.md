...
Canonical artifact = GitHub map [#4]; index = `.work/plans/active/2026-07-31-phase2-wayfinder/`.

Wave 0 tickets (grilling decisions, one per session, order flexible — start with #5):
- **#5** App IA & navigation for Assessments + Practice
- **#6** Assessment types, formats & scoring spec
- **#10** Persistence & local-first fit
...
Work through the Phase 2 wayfinder map (GitHub issue #4, repo rings0fsaturn/study-planner)
in "work through the map" mode. This session: resolve Wave 0, ticket #5 (App IA & navigation for
Assessments + Practice). One decision ticket per session — do NOT resolve more than one.

Context you must load first:
- Read the map #4 (low-res view: destination, Notes, Decisions so far, fog, out-of-scope).
...
- Research already banked: research/external/open-notebook-findings.md (port its patterns, do NOT run
  it as a service) and research/external/code-sandbox-comparison.md (#11 resolved = self-host Judge0 CE).

How to work the ticket:
1. Claim it — assign #5 to me (NotTheRealRohit) before any work.
...
2. Resolve via the grilling + domain-modeling skills: one question at a time, each with your
   ...
   Ground #5 in apps/app/src/pages/, NavBar, AppShell, and the react-router-v7-basename rule.
3. On resolution: post the answer as a resolution comment on #5, close #5, and append a one-line
   pointer to the map #4 "Decisions so far".
4. Update .work: append a dated entry + next-action to
   .work/plans/active/2026-07-31-phase2-wayfinder/VERIFICATION.md, tick #5 in PLAN.md, and refresh
...
After #5, start a fresh session and repeat for #6, then #10 (both independent — can run in
...
- #5, #6, #10 are mutually independent, so they can be run as **parallel sessions** if you want; expect concurrent edits to the tracker and the `.work/` files.
- Wave 0 unblocks nothing directly by itself, but #6 + #10 together unblock **#14** (grading → mastery) later, and #5 frames where everything lives.
