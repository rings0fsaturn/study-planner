...
name: gc-memory
description: >-
  Garbage-collect the code-memory index — remove rows for files that have been deleted, moved, or newly gitignored. Use whenever the user says any variant of "clean up memory", "garbage collect", "gc the index", "remove stale entries", "memory got big", "trim the index", or when Claude notices the in-ToC stale notice ("Note: ~N% of index rows may be stale. Consider running gc-memory."). Also trigger proactively if Claude sees that notice while reading toc.md at the start of a task — surface the option to the user.
---
...
Full sweep over `.claude/memory/index.jsonl`. For each row:
...
```bash
python3 .claude/hooks/gc-memory.py
```
...
- **Does not run automatically.** It's explicit — invoked by the user or by Claude in response to the in-ToC stale notice.
