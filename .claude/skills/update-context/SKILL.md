...
`.claude/memory/` must exist (created by `code-memory`'s `init-memory.sh`). If it doesn't, tell the user `code-memory` needs to be set up first and stop.
...
```bash
for f in .claude/memory/flows/*.md; do
  head -25 "$f"; echo "---"
done
```
...
Path: `.claude/memory/flows/<slug>.md` where `<slug>` is short kebab-case derived from the entrypoint or question (e.g., `login-token-refresh-on-401`).
...
<Narrative trace, in Claude's own words. Reference symbols as `path:line`
so a future Claude can grep `index.jsonl` to verify they still exist.
...
<Optional. Things that surprised Claude during the trace: silent defaults,
...
For files Claude *actually read this session* (not the whole index):

- If a file's index row has `"summary_provisional": true`, upgrade it now via `set-summary.py` with what Claude learned about the file's purpose.
- If a file no longer exists on disk (`test -f`), remove its row from `index.jsonl`.

Then regenerate the ToC:

```bash
python3 .claude/hooks/regen-toc.py
```
...
- **Does not synthesize flows from files not actually read this session.** No making up traces from the index. If Claude didn't read it this session, it doesn't go in a flow file.
