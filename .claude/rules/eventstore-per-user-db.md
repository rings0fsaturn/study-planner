...
Never do this instead: `new Dexie('StudyTracker')` shared across all users, or
`await eventStore.wipe()` on account switch (permanently destroys the previous user's data).
