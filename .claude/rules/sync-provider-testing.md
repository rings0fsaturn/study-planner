...
```ts
// WRONG — returns EventStore instance only; created.eventStore is undefined in beforeEach
...
```ts
// WRONG — the engine is created inside useEffect, so there's no instance to spy on
...
```ts
// WRONG — jsdom never fires visibilitychange/pagehide/beforeunload automatically; waitFor times out
...
```ts
// WRONG — React 19 strict mode runs useEffect mount → cleanup → mount; a bare dispatch
// can land in the gap where listeners are detached
...
```ts
// WRONG — calling waitFor before the dispatch polls a condition that hasn't happened yet
